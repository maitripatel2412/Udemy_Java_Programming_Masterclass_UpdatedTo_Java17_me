package Section_15.Challenge.FinalCollectionsChallenge;
...
public class Store {
    ...
    private void manageStoreCarts() {
        ...
        Cart cart3 = new Cart(Cart.CartType.VIRTUAL, 1);
        carts.add(cart3);

        cart3.addItem(inventory.get("R777"),998);
//        cart3.addItem(inventory.get("B100"),10);
    ...
    private void abandonCarts() {
        ...
        Cart abondonedCart = null;
        while ((abondonedCart = oldCarts.pollFirst()) != null){
            for (String sku : abondonedCart.getProducts().keySet()){
                InventoryItem item = inventory.get(sku);
                item.releaseItem(abondonedCart.getProducts().get(sku));
            }
        }
    }
